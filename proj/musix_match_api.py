...
class Musix:
    ...
    def get_lyrics_alternative(self, title, artist, duration=None):
        ...
        with open(self.token_file, 'r') as file:
            token = json.load(file)['user_token']
        try:
            formatted_url = f"{self.lyrics_alternative}&usertoken={token}&q_album=&q_artist={artist}&q_artists=&track_spotify_id=&q_track={title}&q_duration={duration}"
        except:
            formatted_url = f"{self.lyrics_alternative}&usertoken={token}&q_album=&q_artist={artist}&q_artists=&track_spotify_id=&q_track={title}"
        result = self.get(formatted_url)
